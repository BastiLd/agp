...
const Api = (() => {
  ...
  let zustand = {
    ...
    zeitzone: '',
    serverZeit: '',
    datenSchreibbar: true,   // false = Server kann nichts dauerhaft speichern
    datenFehler: null,
    kiBereit: false,         // Ollama erreichbar und Modell vorhanden
    kiModell: ''
  };
  ...
  function pruefen() {
    ...
    return ruf('api/info')
      .then((antwort) => {
        ...
        zustand.datenSchreibbar = antwort.datenSchreibbar !== false;
        zustand.datenFehler = antwort.datenFehler || null;
        zustand.letzterFehler = null;
        melde();
        if (!zustand.datenSchreibbar) Bus.ruf('serverSpeicherProblem', zustand);
        // Im Hintergrund schauen, ob der KI-Teil nutzbar ist
        kiPruefen().then((k) => {
          const bereit = !!(k && k.ok && k.modellDa);
          if (bereit !== zustand.kiBereit) {
            zustand.kiBereit = bereit;
            zustand.kiModell = (k && k.modell) || '';
            melde();
          }
        }).catch(() => {});
        return true;
  ...
  /* --- KI-Assistent ------------------------------------------------------- */
  function kiPruefen() { return ruf('api/ki'); }
  function kiFragen(satz) {
    return ruf('api/ki', { method: 'POST', body: JSON.stringify({ satz }) });
  }
  ...
  return {
    ...
    planHochladen, schlummern, status, starten, kiPruefen, kiFragen,
