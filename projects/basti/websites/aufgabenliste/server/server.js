...
const VERSION = '2.2.0';   // muss zu APP_INFO.version in public/assets/js/data.js passen

const GRENZE_NORMAL = 2 * 1024 * 1024;      // 2 MB
const GRENZE_FOTOS = 48 * 1024 * 1024;      // 48 MB (bis 10 Fotos pro Tag)

/* ==========================================================================
   Kleine Dateiablage (atomar schreiben, damit nichts halb geschrieben wird)
   ========================================================================== */
/** Legt den Datenordner an. Wirft bewusst nie – ein nicht anlegbarer
 *  Ordner darf den Server nicht am Starten hindern, er soll laufen und
 *  stattdessen deutlich warnen. */
...
/**
 * Kann in den Datenordner überhaupt geschrieben werden? Wenn nicht, geht
 * bei jedem Neustart alles verloren – deshalb wird das beim Start geprüft
 * und laut gemeldet, statt still zu scheitern.
 */
function schreibbarPruefen() {
  try {
    if (!sicherstellen()) return { ok: false, grund: 'Ordner ' + DATEN + ' lässt sich nicht anlegen.' };
    const probe = path.join(DATEN, '.schreibprobe');
...
const SCHREIBBAR = schreibbarPruefen();
if (!SCHREIBBAR.ok) {
  console.error('');
  console.error('  ####################################################################');
  console.error('  #  ACHTUNG: In den Datenordner kann nicht geschrieben werden!      #');
  console.error('  ####################################################################');
  console.error('');
  console.error('  Ordner: ' + DATEN);
  console.error('  Grund:  ' + SCHREIBBAR.grund);
  console.error('');
  console.error('  Folge: Haekchen, Notizen, Fotos und die Anmeldungen fuer');
  console.error('  Benachrichtigungen gehen bei jedem Neustart verloren, und der');
  console.error('  Push-Schluessel wird jedes Mal neu erzeugt.');
  console.error('');
  console.error('  Loesung auf dem Server (Ordner dem Container-Benutzer geben):');
  console.error('      sudo chown -R 1000:1000 "' + DATEN + '"');
  console.error('');
}
    ...
    try {
      ...
      if (weg === '/api/info' && anfrage.method === 'GET') {
        jsonAntwort(antwort, 200, {
          ...
          datenSchreibbar: SCHREIBBAR.ok,
          datenFehler: SCHREIBBAR.ok ? null : SCHREIBBAR.grund,
      ...
      if (weg === '/api/fotos' && anfrage.method === 'POST') {
        ...
        fotos = merge.fotosZusammenfuehren(fotos, rein.fotos || rein, 10);
      ...
      /* KI: laeuft Ollama und ist das Modell da? */
      if (weg === '/api/ki' && anfrage.method === 'GET') {
        jsonAntwort(antwort, 200, await ki.pruefen());
        return;
      }

      /* KI: Satz in einen Aufgabenvorschlag umwandeln */
      if (weg === '/api/ki' && anfrage.method === 'POST') {
        const rein = await koerperLesen(anfrage, GRENZE_NORMAL);
        try {
          const ergebnis = await ki.auswerten(rein.satz, heuteIso(), plan.tageListe());
          jsonAntwort(antwort, 200, ergebnis);
        } catch (fehler) {
          jsonAntwort(antwort, 200, { ok: false, fehler: fehler.message });
        }
        return;
      }
