...
class SearchApp:
    ...
    def perform_search(self):
        ...
        if self.active_filter.get() == "Wörter":
            # Instead of filtering windows based on query, list all open windows.
            windows = gw.getAllTitles()
            if not windows:
                messagebox.showinfo("No Results", "No open windows found.")
                return
                
            # Populate the main results list with all window titles.
    ...
    def open_selection_window(self, windows, query):
        ...
        # Double-clicking a window name will trigger the highlighting process.
        listbox.bind("<Double-Button-1>", lambda event: self.on_window_select(event, listbox, query, sel_win))
        
    def on_window_select(self, event, listbox, query, sel_win):
