...
export class MediaDatabase {
  ...
  getSettings(): AppSettings {
    return {
      ...
      plexUrl: this.getSetting('plexUrl'),
      plexToken: this.getSetting('plexToken'),
      moviesRoot: this.getSetting('moviesRoot'),
      seriesRoot: this.getSetting('seriesRoot')
    };
  }
  ...
  saveSettings(settings: AppSettings): AppSettings {
    ...
    this.setSetting('plexUrl', (settings.plexUrl ?? '').trim());
    this.setSetting('plexToken', (settings.plexToken ?? '').trim());
    this.setSetting('moviesRoot', (settings.moviesRoot ?? '').trim());
    this.setSetting('seriesRoot', (settings.seriesRoot ?? '').trim());
...
async function loadSqlJs(): Promise<SqlJsStatic> {
  // Resolve the sql.js dist directory independently of the current working
  // directory so the WASM binary is found inside the Docker container too.
  // sql.js' "exports" map does not expose package.json, so we resolve the main
  // entry (dist/sql-wasm.js) and use the directory it lives in.
  const requireFromHere = createRequire(import.meta.url);
  const wasmDirectory = dirname(requireFromHere.resolve('sql.js'));
  return initSqlJs({
